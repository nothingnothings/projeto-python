...
def load_keys():
    ...
    response = {
            'message': 'Failed to load existing wallet. Loaded a new one.',
            'public_key': wallet.public_key,
            'private_key': wallet.private_key,
            'funds': 0
...
def get_chain():
    if (not wallet.public_key != None):
        ...
        # return jsonify(response), 200
        return 'Please load or create a new wallet.', 403


    chain_snapshot = blockchain._chain
...
#     block = blockchain._chain[-1].__dict__.copy()
...
def broadcast_block():
    ...
    if block['index'] == blockchain._chain[-1].index + 1:
        ...
        blockchain.resolve_conflicts = True ##é o próprio peer node que deverá tentar resolver esse conflict...
        return jsonify(response), 200
    else:
